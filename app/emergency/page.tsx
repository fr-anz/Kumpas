...
export default function EmergencyPage() {
  ...
  return (
    <div className="flex flex-col gap-6 page-enter">
      ...
      <div className="rounded-card border border-danger bg-surface p-5 shadow-[2px_2px_0px_0px_rgba(215,38,61,1)]">
      ...
      {profile && !editing ? (
        <>
          ...
          <button
            ...
            className="flex min-h-12 items-center justify-center gap-2 rounded-button border border-border-lining bg-surface px-6 text-base font-bold transition-all shadow-[2px_2px_0px_0px_var(--shadow-color)] hover:bg-surface-alt active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_var(--shadow-color)]"
          >
            <svg
          ...
          <button
            ...
            className="flex min-h-12 items-center justify-center rounded-button border border-border-lining bg-surface px-6 text-lg font-bold transition-all shadow-[2px_2px_0px_0px_var(--shadow-color)] hover:bg-surface-alt active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_var(--shadow-color)]"
          >
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          ...
          <label className="flex flex-col gap-1.5">
            <span className="font-bold">ID Photo (Optional)</span>
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  reader.onload = (ev) => {
                    setDraft({ ...draft, photoBase64: ev.target?.result as string });
                  };
              ...
              className="rounded-button border border-border-lining bg-surface p-2.5 text-sm shadow-[2px_2px_0px_0px_var(--shadow-color)] transition-all"
            />
            {draft.photoBase64 && (
              <img src={draft.photoBase64} alt="Preview" className="mt-2 h-24 w-24 rounded-full border border-border-lining object-cover" />
            )}
          </label>
          <Field
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className="min-h-12 rounded-button border border-border-lining bg-surface px-4 text-lg shadow-[2px_2px_0px_0px_var(--shadow-color)] focus:outline-none transition-all"
      />
    </label>
  );
}
...
function ContactField({
  ...
  return (
    <label className="flex flex-col gap-1.5">
      <span className="font-bold">{label}</span>
      <div
        className={`flex min-h-12 items-center gap-2 rounded-button border bg-surface pl-3 pr-4 shadow-[2px_2px_0px_0px_var(--shadow-color)] focus-within:outline-none transition-all ${
          showError
            ? "border-danger"
            : validNumber
              ? "border-success"
