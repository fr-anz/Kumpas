...
def main():
    ...
    with open(labels_csv_path, "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        for row in reader:
            if len(row) > 0 and row[0] == args.folder_name:
                labels_exist = True
                break
    if not labels_exist:
        with open(labels_csv_path, "a", encoding="utf-8", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([args.folder_name, label_name, args.category])
        print("Updated labels.csv")
    else:
        print(f"{args.folder_name} already in labels.csv")
    ...
    with open(train_csv_path, "a", encoding="utf-8", newline='') as f:
        ...
        for video in videos:
            ...
            if vid_rel_path not in existing_vids:
                writer.writerow([vid_rel_path, args.folder_name, label_name, args.category])
                new_entries += 1
                
    print(f"Added {new_entries} new videos to train.csv")

    if new_entries == 0 and labels_exist and label_name in config.get("labels", []):
